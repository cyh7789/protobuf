...
def _AddDependentFileDescriptorProtos(
    pool_instance, file_descriptor, dependency_set
):
  """Adds dependent file descriptors to the pool."""
  if file_descriptor.name in dependency_set:
    return
  try:
    pool_instance.FindFileByName(file_descriptor.name)
    dependency_set.add(file_descriptor.name)
    return
  except KeyError:
    pass

  for dependency in file_descriptor.dependencies:
    _AddDependentFileDescriptorProtos(pool_instance, dependency, dependency_set)
...
def MakeProtoClassWithProtoFields(fields, full_name=None, pool=None):
  ...
  for f_name, f_type in field_items:
    fields_hash.update(f_name.encode('utf-8'))
    if hasattr(f_type, 'DESCRIPTOR') and isinstance(
        f_type.DESCRIPTOR, descriptor.Descriptor
    ):
      fields_hash.update(f_type.DESCRIPTOR.full_name.encode('utf-8'))
    else:
      fields_hash.update(str(f_type).encode('utf-8'))
  proto_file_name = fields_hash.hexdigest() + '.proto'
  ...
  for _, f_type in field_items:
    if hasattr(f_type, 'DESCRIPTOR') and isinstance(
      ...
      _AddDependentFileDescriptorProtos(
          pool_instance, f_type.DESCRIPTOR.file, dependency_set
      )
