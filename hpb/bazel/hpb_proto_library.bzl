...
def _compile_hpb_protos(ctx, proto_info, proto_sources, generate_meta_data):
    ...
    if generate_meta_data:
        out_flag = toolchain.out_replacement_format_flag
        if "backend=cpp" in out_flag:
            out_flag = out_flag.replace("backend=cpp", "backend=cpp,annotate_headers")
        else:
            out_flag = out_flag.replace("--hpb_out=", "--hpb_out=annotate_headers:")
...
def _upb_cc_proto_aspect_impl(target, ctx, cc_provider, file_provider):
    generate_meta_data = ctx.attr._emit_proto_annotation_metadata[BuildSettingInfo].value
...
_hpb_proto_library_aspect = aspect(
    attrs = {
        ...
        "_emit_proto_annotation_metadata": attr.label(
            default = "//bazel/flags:emit_proto_annotation_metadata",
        ),
